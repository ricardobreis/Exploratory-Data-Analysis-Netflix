...
ggplot(progressao_serie, aes(x = as.factor(ano_mes), y = n, group = 1)) +
  geom_line() +
  labs(
    x = "Mês-Ano",
    y = "Episódios"
...
progressao_filme <- historico_filme %>%
  group_by(ano_mes) %>%
  count()

ggplot(progressao_filme, aes(x = as.factor(ano_mes), y = as.factor(n), group = 1)) +
  geom_line() +
  labs(
    x = "Mês-Ano",
    y = "Episódios"
